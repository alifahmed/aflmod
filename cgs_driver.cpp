//===- cgs_driver.c - a glue between AFL and libFuzzer --------*- C++ -* ===//
//
...
/* This file is based on afl_driver.cpp from llvm-project */
...
static int ExecuteFilesOnyByOne(int argc, char **argv) {
  for (int i = 1; i < argc; i++) {
    ...
    assert(fd > -1);
    ssize_t n_read = read(fd, AflInputBuf, kMaxAflInputSize);	
    if (n_read > 0) {
